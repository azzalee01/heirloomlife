...
export default function PlatformPreview() {
  ...
  // ── take over from demo when user interacts anywhere in the preview ──────
                  ...
                  {showAmendments && !userMode && (
                    <div style={{ borderRadius: 8, border: '1px solid var(--teal)', background: '#fff', overflow: 'hidden', flexShrink: 0, animation: 'slideUpCard .4s ease' }}>
                      <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--line)', background: 'rgba(42,180,174,.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--teal-deep)' }}>2 amendments suggested</span>
                        <span style={{ fontSize: 9, color: 'var(--neutral)' }}>Pending your approval</span>
                      </div>
                      {DEMO_AMENDMENTS.map((a, i) => (
                        <div key={i} style={{ padding: '8px 14px', borderBottom: i < DEMO_AMENDMENTS.length - 1 ? '1px solid var(--line)' : 'none', animation: `slideUpCard .4s ease ${i * 100 + 100}ms both` }}>
                          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink)' }}>{a.title}</div>
                          <div style={{ fontSize: 9, color: 'var(--neutral)', marginTop: 2 }}>{a.detail}</div>
                        </div>
                      ))}
                      <div style={{ padding: '10px 14px' }}>
                        <button
                          onClick={e => e.stopPropagation()}
                          style={{ width: '100%', padding: '7px', borderRadius: 6, background: phase === 'applied' ? '#ecfdf5' : 'var(--ink)', color: phase === 'applied' ? '#065f46' : '#fff', fontSize: 11, fontWeight: 700, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, animation: approvePulse ? 'pulseGlow 1s ease infinite' : 'none', transition: 'background .4s ease, color .4s ease' }}
                        >
                          {phase === 'applied'
                            ? <><Icon d="M20 6L9 17l-5-5" color="#065f46" size={13}/> Amendments approved</>
                            : phase === 'approving'
                            ? 'Approving…'
